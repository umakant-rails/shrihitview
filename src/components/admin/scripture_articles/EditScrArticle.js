...
import { getScrArticle, createScrArticle, updateScrArticle } from '../../../actions/admin/admin_scr_articles';

const articleObj = {
  scripture_id: '', chapter_id: '', article_type_id: '', 
  index: '', content: '', interpretation: ''};

const EditScrArticle = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const {scripture_id, id} = useParams();
  ...
  useEffect( () => {
    if(scripture_article){
      setFormValues( formValues => ({
        ...
        content: scripture_article.content,
        interpretation: scripture_article.interpretation
      }));
