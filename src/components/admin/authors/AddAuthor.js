...
const AddAuthor = () => {
  ...
  const { sampradayas, authorCreated, sampradayaCreated } = useSelector( (state) => state.adminAuthor)
  ...
  useEffect( () => {
    if(authorCreated){ resetForm();/*navigate('/articles/new');*/ } 
    if(sampradayaCreated){
      setSampradayaFormDisplay(false);
    }
  }, [authorCreated, sampradayaCreated]);
