...
const StoryScripturePage = () => {
  ...
  const [totalStories, setTotalStories] = useState(total_stories);
  const [currentPage, setCurrentPage] = useState(current_page || 1);
  const [searchAttr, setSearchAttr] = useState({page: 1});

  useEffect( () => {
    dispatch(getScrStories({id:id, params: searchAttr}));
  }, [dispatch, id]);
  ...
  const deleteToScrStory = (story_id) => {
    const isTrue = window.confirm("Are you sure you want to delete this record ?");
    if(isTrue){
      dispatch(deleteScrStory({id:scripture.id, story_id:story_id, params: searchAttr})); 
    }
  }
                    ...
                    storiesList && storiesList.length > 0 ? storiesList.map( (story, index) => 
                      <tr key={index}
                        ...
                        <td className="px-2 py-3">
                          <Link to={`/stories/${story.id}`} className="text-blue-600" target="_blank">
                            {story.title}
                          </Link>
                        </td>
                        <td className="px-2 py-3">
                        ...
                        <td className="px-2 py-3 flex items-center  justify-center">
                          ...
                          <Link to="#" onClick={e => deleteToScrStory(story.id)}>
