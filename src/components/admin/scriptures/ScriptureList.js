...
import { getScriptures, deleteScripture, updateScripture } from '../../../actions/admin/admin_scriptures';
...
const ScriptureList = () => {
  ...
  const [totalScriptureQnty, setTotalScriptureQnty] = useState(0);
  ...
  useEffect( () => {
    if(scriptures){
      ...
      setTotalScriptureQnty(total_scriptures);
                    ...
                    scriptureList.length > 0 ? scriptureList.map( (scripture, index) => 
                      <tr key={index}
                        ...
                        <td className="px-2 py-3 flex items-center  justify-center">
                          <Link to={`/admin/strota/${scripture.id}/edit`}>
