...
import { getPanchang } from '../../../actions/admin/admin_panchangs';
import { useDispatch, useSelector } from 'react-redux';
import { ReactTransliterate } from 'react-transliterate';
//https://tailwindcomponents.com/component/calendar-4

const tithiObj = {date: '', tithi: '', paksh: '', description: '', title: '', year: ''}
const AddTithi = () => {
	...
	return (
      ...
      <div className='col-start-2 col-span-10 shadow-2xl bg-white border border-gray-200 px-10 pt-5 pb-8'>
          ...
          <div className='col'>
            <header className="flex items-center justify-between border-b border-gray-200 px-6 py-4 lg:flex-none">
              <h1 className="text-xl font-semibold leading-6 text-gray-900">
                <time dateTime="2022-01">{panchangObj ? `${`विक्रम सम्वत ${panchangObj.vikram_samvat}`}` : null}</time>
              </h1>
            </header>
            <div className="shadow ring-1 ring-black ring-opacity-5">
              <div className={`grid grid-cols-7 gap-px border-b border-gray-300 bg-gray-200 text-center 
                    ...
                    PAKSH.map( (paksh, index1) => (
                      TITHIS.map( (tithi, index2) => (
                        <button key={`${index1}${index2}`} type="button" 
                          className={`flex h-14 flex-col px-3 py-2 hover:bg-yellow-600 focus:z-10
                            ${paksh === PAKSH[0] ? 'bg-black' : 'bg-white' }`}>
        ...
        <div className='grid md:grid-cols-2 gap-6 border-y py-4 mb-4 border-gray-400'>
          <div className='col'>
            ...
            <div className="mb-2">
              ...
              <label className="block mb-2 h-9 font-medium text-gray-900 border border-gray-400 px-2 py-1 rounded">
                {formValues.paksh || ''} {formValues.tithi || ''}
              </label>
            </div>
          </div>
          <div className='col'>
            <div className="mb-5">
              ...
              <label className="block mb-2 font-medium text-gray-900 border border-gray-400 px-2 py-1 rounded">
                {formValues.hindi_month || ''}, {formValues.paksh || ''} {formValues.tithi || ''}
              </label>
            </div>
            <div className="mb-2">
              <label className="block mb-2 font-medium text-gray-90 font-bold">
